...
namespace Grid 
{
    ...
    struct CGrid<1>
    {
        ...
        */
        CGrid(double x0_, double x1_, int nx_, double tmin_, double tmax_, int nt_) :
             x0(x0_), x1(x1_), nx(nx_), t0(tmin_), t1(tmax_), nt(nt_)
             {
                dx = (x1-x0)/(nx);
                dt = (t1 - t0) / (nt);
                ...
                if(nx % 2 != 0)
                {
                    std::cout << "One of the Griddimensions is not divisible by 2!" << std::endl;
                    std::exit(1);
                }
             };
    };
    ...
    struct CGrid<2>
    {
         ...
         * @param nt_
         */
        CGrid(double x0_, double x1_, double y0_, double y1_, int nx_, int ny_, double tmin_, double tmax_, int nt_)
